...
struct TINA_SHARE {
  ...
  /*
    ...
    For more details see comment to ha_inverted::update_status.
  */
  bool is_log_table;
  /*
...
class ha_inverted : public handler {
 ...
 public:
  ...
  ~ha_inverted() override {
    if (chain_alloced) my_free(chain);
    if (file_buff) delete file_buff;
    blobroot.Clear();
  }
  const char *table_type() const override { return "INVERTED"; }
  ...
  */
  ...
  std::vector<my_off_t> searchInvertedList(const std::unordered_map<std::string, std::vector<my_off_t>>& dictionary, const std::vector<std::string>& input);
