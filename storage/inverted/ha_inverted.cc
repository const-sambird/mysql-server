...
// Search dictionary given list of terms
// Returns all records containing all given terms
vector<my_off_t> ha_inverted::searchInvertedList(const unordered_map<string, vector<my_off_t>>& dictionary, const vector<string>& input) {
    vector<my_off_t> hitRecords;
    
    // Find matches for first term
    auto hits = dictionary.find(input[0]);
    if (hits == dictionary.end()) {return {};}
    hitRecords = hits->second;
    
    // Find matches for rest of terms
    for (size_t i = 1; i < input.size(); ++i) {
        hits = dictionary.find(input[i]);
        if (hits == dictionary.end()) {return {};}
        auto& values = hits->second;

        vector<my_off_t> temp;
        set_intersection(hitRecords.begin(), hitRecords.end(), values.begin(), values.end(), back_inserter(temp));
